...
app = Flask(__name__)
openai.api_key = "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"

def generate_images(prompt):
    response = openai.Image.create(prompt=prompt, n=3, size="1024x1024")
    img_urls = [data['url'] for data in response['data']]
    file_names = ['image.jpg', 'image2.jpg', 'image3.jpg']
    for idx, url in enumerate(img_urls):
        download_image(url, file_names[idx])
    return file_names
...
@app.route('/prompt_result', methods=['POST'])
def prompt_result():
    if request.method == 'POST':
        user_prompt = request.form['prompt']
        file_names = generate_images(user_prompt)
        indexed_images = [{'index': idx + 1, 'path': image} for idx, image in enumerate(file_names)]
        return render_template('result1.html', images=indexed_images)

    
@app.route('/display', methods=['POST'])
def display():
    if request.method == 'POST':
        img_num = int(request.form['image'])
        if 1 <= img_num <= 3:
            chosen_image = f'image{img_num}.jpg'
            img = plt.imread(chosen_image)
            plt.imshow(img)
            plt.axis('off') 
            plt.title(f"Chosen Image {img_num}")
            plt.show()
            return render_template('display.html', img_path=chosen_image)
        else:
...
def upload():
    if request.method == 'POST':
        ...
        return render_template("result.html",html_data = html_data , result=result, cmt=percentage)
...
def results():
    ...
    if len(filtered_foods) > 0:
        ...
        return render_template("result.html", html_data = calorie , result=result, cmt=cmt)
